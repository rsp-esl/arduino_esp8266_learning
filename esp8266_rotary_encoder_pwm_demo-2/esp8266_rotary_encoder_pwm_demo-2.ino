...
const int PB_PIN = 2;       // D4 = GPIO2  (SW pin of the rotary encoder switch, pusb button, active-low)
const int PWM_LED_PIN = 4;  // D2 = GPIO4
...
void setup() {
  ...
  attachInterrupt( QA_PIN, encode_inputs, CHANGE );
  attachInterrupt( PB_PIN, button_press_action, FALLING );
  ...
  analogWriteFreq( 500 ); // set PWM frequency: 500Hz 
...
void loop() {
  if ( button_pressed ) {
     ...
     encoder_cnt = 0;
     Serial.println( F("Reset encoder counter") );
